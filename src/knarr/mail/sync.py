    ...
    async def _push_to_peer_inner(self, peer_node_id: str, peer_host: str, peer_port: int):
        ...
        try:
            ...
            if resp is None:
                ...
                # B4: mail_delivery_receipt (nak) — null response, will retry
                if hasattr(self._node, '_write_receipt'):
                    _dur_ms_null = int((time.time() - _push_start) * 1000)
                    self._node._write_receipt(
                        document_type="mail_delivery_receipt",
                        payload={
                            "sender": self._node._public_key_hex,
                ...
                # B4: mail_delivery_receipt (ack) — batch delivered successfully
                if hasattr(self._node, '_write_receipt'):
                    _dur_ms = int((time.time() - _push_start) * 1000) if '_push_start' in dir() else 0
                    self._node._write_receipt(
                        document_type="mail_delivery_receipt",
                        payload={
                            "sender": self._node._public_key_hex,
                ...
                # B4: mail_delivery_receipt (nak) — unexpected response type
                if hasattr(self._node, '_write_receipt'):
                    _dur_ms_unexp = int((time.time() - _push_start) * 1000)
                    self._node._write_receipt(
                        document_type="mail_delivery_receipt",
                        payload={
                            "sender": self._node._public_key_hex,
                            ...
                            "delivery": {
                                ...
                                "attempt": 1,
                                "endpoint": f"{peer_node_id[:8]}@tcp",
                                "duration_ms": _dur_ms_unexp,
                                "error": f"unexpected_{type(resp).__name__}",
                            },
                        },
                        counterparty=peer_node_id,
                        order_ref=item_ids[0] if item_ids else None,
            ...
            await self._node._enqueue_write(self._node.storage.mark_outbox_pending, item_ids)
            # B4: mail_delivery_receipt (term) — exception during push, terminal failure record
            if hasattr(self._node, '_write_receipt'):
                _dur_ms_exc = int((time.time() - _push_start) * 1000)
                self._node._write_receipt(
                    document_type="mail_delivery_receipt",
                    payload={
                        "sender": self._node._public_key_hex,
        ...
        for item in msg.items:
            ...
            if stored:
                ...
                if hasattr(self._node, '_write_receipt'):
                    ...
                    _body_str = json.dumps(item.get("body")) if isinstance(item.get("body"), dict) else str(item.get("body", ""))
                    ...
                    self._node._write_receipt(
                        ...
                        payload={
                            "receiver": self._node._public_key_hex,
