...
On-chain payment detection and receipt generation for Solana (poll-only).
...
In your `knarr.toml`, the plugin ships with a default `plugin.toml` that
configures Solana mainnet. Override in your node config if needed:
...
For **testnet/devnet**, change the `chain_id` and `rpc_url`:
...
The plugin accepts any `chain_id` starting with `solana-`.
...
- `bcw_watchlist` — address-to-node mapping with cursor tracking
- `bcw_seen` — deduplication table (chain_id + tx_hash + tx_index)
- `bcw_receipts` — signed receipt documents with dedup keys

## Known limitations

- Poll-only (no WebSocket streaming)
- `chain_id` in `plugin.toml` defaults to `solana-mainnet`; testnet/devnet
  requires changing the config (see above)
- RPC rate limits may apply on public endpoints; consider a dedicated RPC
