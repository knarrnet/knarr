...
class ConnectionPool:
    ...
    async def _refill_peer_pubkey_async(self, peer_id: str) -> None:
        ...
        try:
            loop = asyncio.get_running_loop()
            pub_hex = await loop.run_in_executor(self._pubkey_lookup_pool, lookup, peer_id)
        except Exception:
            return
        if not pub_hex:
            return
    ...
    async def _try_send(self, reader: asyncio.StreamReader,
        ...
        try:
            ...
            if response is None:
                return _SEND_FAILED
            # v0.57.0 A-1: WeakKeyDictionary stash removed; pool responses do not
            # carry transport metadata via side-table. Cert pinning on pool paths
            # falls back to TransportMetadata() (no-op check). Follow-on sprint
            # to add explicit pool-level cert verification.
            return response
        except Exception:
            return _SEND_FAILED
    ...
    async def close_all(self):
        ...
        for peer_id in list(self._pool.keys()):
            await self._close_conn(peer_id)
        self._pubkey_lookup_pool.shutdown(wait=False)
