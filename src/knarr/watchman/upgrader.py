...
def _rename_watchman_exe_for_upgrade() -> Optional[str]:
    """B-03: On Windows, rename the locked knarr-watchman.exe before package reinstall.

    The package manager (pip) cannot overwrite a locked .exe on Windows via
    --force-reinstall. Renaming a locked file IS allowed on Windows — the OS
    deletes the renamed file once the last handle closes (on process exit).
    The new .exe is written cleanly into the original path.
